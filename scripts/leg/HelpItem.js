...
define("jquery/helpTipPopup", ["jquery"], function ($) {
    $.fn.tipHelpBox = function(helpBoxOptions) {
        return $(this).each(function () {		
            ...
            $(".btn-help-close", helpbox).click(function(e) {
                    e.preventDefault(); // disable anchor link
                    helpbox.animate({opacity: "hide"}, defaults.displaySpeed);
            });
            ...
            var popup = function(e){
                ...
                */
                ...
                (option.vertiBoxPos == 'top') ?  topShowPosition = 0-(helpbox.outerHeight()+10) : topShowPosition = -4;
                ...
                // Set the options for to the side middle positioned boxes
                if (option.horizBoxPos == 'left') {
                        leftPosition = 0-(helpbox.outerWidth()) - 7;
                        if(option.vertiBoxPos == 'bottom') {
                            topShowPosition = -5;
                        } else if (option.vertiBoxPos == 'top') {
                            topShowPosition = 0-(helpbox.outerHeight()-16);
                        }
                }
                ...
                if (helpbox.css("display") == "none") {
                        ...
                        helpbox.animate({opacity: "show", left: offset["left"] + leftPosition, top: offset["top"] + topShowPosition}, defaults.displaySpeed);
                } else {
                        helpbox.animate({opacity: "hide", left: offset["left"] + leftPosition, top: offset["top"] + topHidePosition}, defaults.displaySpeed);
                }

                function addClasses(helpbox) {
